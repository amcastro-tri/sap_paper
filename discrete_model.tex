...
t^{n+1}+(1-\theta)t^{n}$, with $\theta \in [0, 1]$. \reviewquestion{Q7}{We define \emph{mid-step
...
\reviewquestion{Q7}{This scheme based on the $\theta\text{-method}$ includes some of the most
...
Euler scheme, implicit in the $\mf{q}$ update in \eqref{eq:scheme_q_update}
while explicit in $\mf{k(\mf{q}, \mf{v})}$ in the momentum balance
\eqref{eq:scheme_momentum}.}
