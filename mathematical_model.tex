...
\RedHighlight{This opening paragraph repeated itself, twice mentioning
soft-body models and springs and dampers. I took a pass to remove this repetition.
I also changed some language that I found awkward. Pick and choose the changes that you like.}.

Our discrete-time model is based on the $\theta\text{-method}$ \RedHighlight{Add citation to this method}
...
We solve Eqs. (\ref{eq:v_update}) and (\ref{eq:q_update}) in two stages. (\RedHighlight{Does this two-stage technique have a name? If so, add ref}) In the
...
(\RedHighlight{what is meant by this? what other "schemes" might we use?}), we
...
constraints do not produce work. \RedHighlight{Unclear what "no work" means in the context of the discrete-time model.  Do we mean that 
$\langle F(q^{\theta}), v^{\theta}\rangle = 0$?} This is the case of equality constraints and
rolling friction for instance. That is, we'd expect very good \RedHighlight{Dislike "very good". Can you replace this with a quantitative statement?} energy
...
\RedHighlight{Recommend never introducing N if it is always the identity} which is commonly the case for degrees of
...
elements to matrix $\mf{A}$, increasing the stability of the scheme.\RedHighlight{Unclear what "increase stability" means}
...
gyroscopic terms. \RedHighlight{This a consequence of working in momentum-space, yes? In other words,
a scheme based on Gauss's principle, that solves for accelerations instead of velocities, would always yield convex problem. Just asking for my own clarification...}
...
\RedHighlight{What does "treated implicitly" mean? In what way is their derivative is zero in \ref{eq:dissipation_matrix}? When you introduced $F(q,v)$
you said that it already includes gyroscopic and Corolis terms, but this discussion seems to about how to add them, as if they weren't already there...  }
