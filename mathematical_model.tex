...
\end{align}
Furthermore, in the absence of impact, which we characterize by $\mf{v} -
\mf{v}^* = \mathcal{O}(dt)$, the approximation in Eq.
(\ref{eq:momentum_linearized}) and the original discrete momentum update in Eq.
(\ref{eq:v_update}) agree to second order as shown by the following result.
\begin{prop}	
In the absence of impact, $\mf{A}$ is a first order approximation to the
Jacobian of $\mf{m}$, i.e.,
\begin{align}
...
the discrete balance of momentum in Eq. (\ref{eq:v_update}). \RedHighlight{TODO:
consider replacing this sentence with $\mf{m}(\mf{v})$ and
$\mf{A}(\mf{v}-\mf{v}^*)$ agree to second order as suggested by Frank.}
Moreover, $\mf{A} \succ 0$.
\label{prop:gradient_of_m_approximation}
\end{prop}
\RedHighlight{Consider discussing the case where impact exists.}
...
for bodies in contact that are under rolling friction but not sliding friction,
the contact constraints behave as bi-lateral constraints that impose zero slip
velocity. In this case, our method conserves energy when it is used with the
symplectic mid-point rule. We demonstrate this in Section XXX with an example
simulation of billiard balls.  \RedHighlight{Alejandro: Add proper reference to
the billiard balls example in the numerical examples section.}
