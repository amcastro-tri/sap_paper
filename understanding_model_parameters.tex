...
We have provided explicit algebraic expressions for the impulses as a function
of contact velocities in Eq. \eqref{eq:analytical_y_projection} via the inverse
...
With the formulation presented in this work, all bodies are modeled as
compliant. Therefore, rigid objects must be modeled as \emph{near-rigid} bodies
with large stiffness. However, as mentioned above, blindly choosing large values
of stiffness can lead to ill-conditioned systems of equations. Here, we propose
a principled way to choose the stiffness parameter when modeling near-rigid
contact.
