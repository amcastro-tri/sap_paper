...
with a small fraction of the total cost. Though both line-search approaches
...
maximum step length of $\alpha_\text{Max}$ and progressively decreases
by a factor $\rho \in (0, 1)$ to $\alpha\gets\rho\alpha$ until Armijo's
...
structure when solving Line~\ref{op:Newton_iteration} at each Newton iteration.
...
will be denoted with $n_t$. A free body is a special case of a tree with
...
in the range from $10^{-6}$ to $10^{-3}$. For all of our simulations in this paper,
...
typically due to external forces. We always set this tolerance to a
