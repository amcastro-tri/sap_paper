...
The Semi-Analytical Primal Solver (SAP) is an iterative algorithm for solving
the unconstrained  formulation (\ref{eq:primal_unconstrained}) that is inspired
by Newton's method.  Exploiting differentiability of the objective
...
of non-differentiability.  As we show \fbox{TODO: add reference}, SAP 
globally convergences at at least a linear-rate.
...
$\mf{J}^T\bgamma(\mf{v})$. The result is
\begin{eqnarray}
...
In total, the Hessian of $\ell_p$ is
\begin{equation}
	\nabla_\mf{v}^2\ell_R(\mf{v})= \mf{A} + \mf{J}^T\mf{G}\,\mf{J}
...
\end{eqnarray}
These is computed efficiently by first calculating the change in velocity
$\Delta\mf{v}_c:=\mf{J}\Delta\mf{v}$ and change of momentum $\Delta\mf{p} :=
\mf{A}\Delta\mf{v}$ by exploiting kinematic sparsity. 
The calculation is then completed via 
\begin{eqnarray}
