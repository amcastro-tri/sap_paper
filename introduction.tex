...
product $\delta t\Vert\vf{v}_t\Vert$ can be neglected.

Todorov \cite{bib:todorov2014} introduces regularization into Anitescu's
formulation in order to write a strictly convex formulation with a unique
solution. Todorov uses regularization to introduce \emph{numerical compliance}
as a means of a Baumgarte-like stabilization and avoid constraints drift.
...
work by Anitescu, state convergence to original problem with step size.
\sout{Cite work from Acary (2011) in whatev context is applied}. Quickly
introduce todorov's saying he came up with regularization so that the problem
has unique solution and that he came up with \emph{inverse dynamics}. Say how he
uses regularization as a means of constraint stabilization, which is different
from our work.}

\CyanHighlight{Brief paragraph on available engineering solvers. Point out that
there are not solvers besides Mujoco that use the convex approximation. Point
out that this has to do in par that it is not know how to solve it effectively,
or similar. Essentially copy what you have in the folder above. State how Mujoco
is possibly the only software that uses the convex approximation of contact,
though the inner workings are not common knowledge in the community. Also state
