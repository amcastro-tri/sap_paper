...
at the next time step. Let us now analyze the resulting impulses from this
model.
...
\textbf{Normal impulses}. We observe that in stiction, we recover the compliant
...
equivalently, $R_t \ll R_n$. Section \ref{sec:conditioning}
...
couples into the normal impulses as $\gamma_n=-\delta t(k(\phi-(\delta
...
when sliding goes to $-k(\phi-\tau_d\mu\Vert\vf{v}_t\Vert)-d\,v_n$ in the limit to
