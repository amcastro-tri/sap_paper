...
\CyanHighlight{Xuchen: Stating the problem and the solution at the same time is
a little cluncky. How about this:
... the impulse $\bgamma_k$ at the $k\text{-th}$ contact point is the solution
to the following convex optimization problem
\begin{eqnarray*}
	\bgamma_k(\vf{v}_{c,k})&=&\begin{aligned}
		\argmin_{\bgamma\in\mathcal{F}_k} \quad &
	\frac{1}{2}(\bgamma-\vf{y}_k)^T\vf{R}_k(\bgamma-\vf{y}_k) \end{aligned}\\
	\vf{y}_k(\vf{v}_{c,k}) &=& -\vf{R}_k^{-1}(\vf{v}_{c,k}-\hat{\vf{v}}_k)    
\end{eqnarray*}
The solution $\bgamma_k(\vf{v}_{c,k})$ can be obtained by projecting
$\vf{y}_k(\vf{v}_{c,k})$ onto the cone $\mathcal{F}_k$ using the norm defined
by $\vf{R}_k$, i.e.
\begin{align*}
	\bgamma_k(\vf{v}_{c,k})= P_{\mathcal{F}_k}(\vf{y}_k(\vf{v}_{c,k}))
\end{align*}}

\CyanHighlight{Xuchen: This seems to me to be the perfect place to insert what you have in
section \ref{sec:unconstrained_convex_formulation}. Otherwise, the reader is left
without clue on why the primal formulation is introduced and why the analytic inverse is of
particular interest.}
