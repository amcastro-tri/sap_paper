...
\reviewquestion{R8-Q1/AE-Q4}{Given a configuration $\mf{q}$ of the system, we
assume our geometry engine reports a set $\mathscr{C}(\mf{q})$ of $n_c$
potential contacts between pairs of bodies. We characterize the $i\text{-th}$
\emph{contact pair} in $\mathscr{C}(\mf{q})$ by the location $\vf{p}_i$ of the
contact point, a normal direction $\hat{\vf{n}}_i$ and \emph{signed distance
function}
