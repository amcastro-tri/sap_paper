...
\begin{eqnarray}
	...
	\label{eq:dual_cost}
\end{eqnarray}
\CyanHighlight{Xuchen: $\mathbf{W} = \mathbf{J}\mathbf{M}^{-1}\mathbf{J}^T$}
where $\mathbf{W} =
...
\CyanHighlight{Xuchen: Could you say more about the linearized friction cone?
Is it necessary for Anitescu's formulation? Also, ``...constraints corresponds
to optimality conditions'' sounds a little weird. It's the really the momentum
quation *and* the constraints that leads to the optimization formulation, right?}
...
\CyanHighlight{Xuchen: From the notation convention, this looks like a per
contact point condition and thus should have the k subscript.}
\begin{equation}
...
using the solution from previous time steps, a serious problem with the
